...
function agregarAlumno(alumnos, notas, cantidadAlumnos) { //Función agregar alumno, es necesario llamar a cantidadAlumnos?
    ...
    for (let a = 0; a < cantidadAlumnos; a++) { //variable alumnos empieza en 0, va a tener tantos elementos como el profe diga
        ...
        if (alumnos.length >= cantidadAlumnos) { // si el largo del array alumnos es mayor a la cantidad de alumnos que dijo el profe
            ...
            if (cedulas.length !== 8) { //Si el largo de la cedula no es igual a 8
                ...
                for (let f = 0; f < alumnos.length; f++) { // variable, comparación, las evecs que itera
                    ...
                    let CIarray = alumnos[f]; // declaramos variable CIarray / f= variable del for
                    //alumnos = //Donde tengo que buscar las CI
                    //cedulas  //El elemento que tengo que buscar
                    if (CIarray === cedulas) {
                        document.write('La cedula ya está ingresada');
                    }
                }
